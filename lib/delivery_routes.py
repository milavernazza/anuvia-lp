...
async def intake_post(practice: str, request: Request):
    ...
    if delivery_mode == "autonomous":
        ...
        body = (
            ...
            'nosso time analisa o material e agenda uma sessão executiva para '
            'apresentar os findings da semana 2 ao vivo (60 min). O convite '
            'chega no email do sponsor — todos os stakeholders listados serão convidados em cópia.</p>'
            '<p class="muted">Próximos passos:</p>'
            '<ol class="muted">'
            '<li>Anuvia trabalha na análise (~3-5 dias)</li>'
            '<li>Apresentação executiva da semana 2 — convite por email</li>'
            '<li>Após a apresentação encaminhamos os materiais (PDF + PPTX) oficialmente</li>'
            '</ol>'
        )
